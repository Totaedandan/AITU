...
def get_links(text):
    ...
    res = requests.get(
        url=f"https://hh.kz/search/vacancy?text={text}&area=159",
    ...
    try:
        page_count = int(soup.find("div",attrs={"class":"pger"}).find_all("span",recursive=False)[-1].find("a").find("span").text)
    except:
        return
    for page in range(page_count):
        try:
            res = requests.get(
                url=f"https://hh.kz/search/vacancy?text={text}&area=159{page}",
                headers={"user-agent":ua.random}
            )
            if res.status_code == 200:
                soup = BeautifulSoup(res.content, "lxml")
                for a in soup.find_all("a",attrs={"class":"resume-search-item__name"}):
                    yield f'https://hh.kz{a.attrs["href"].split("?")[0]}'
...
def get_resume(link):
    ...
    try:
        ...
        name = ""
    try:
        salary = soup.find(attrs={"class":"bloko-header-section-2_lite"}).text.replace("\u2009","").replace("\xa0"," ")
    except:
        salary = ""
    try:
